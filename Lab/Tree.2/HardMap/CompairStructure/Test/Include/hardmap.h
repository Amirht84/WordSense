...
template<typename id,typename data>
void hardMap<id, data>::display_tree(){
	const std::string ErrLog = "err in hardMap<id, data>::display_tree";

	std::function<void(const long long&)> main_engine = [&](const long long& CurrentPos){
		hardNode CurrentNode;
		if(!read_file(CurrentPos, CurrentNode)) throw std::runtime_error(ErrLog + "can't read");
		std::cout << CurrentNode.Id << '\n';
		if(CurrentNode.RightOffset != -1){
			std::cout << "right child of " << CurrentNode.Id << ": ";
			main_engine(CurrentNode.RightOffset);
		}
		if(CurrentNode.LeftOffset != -1){
			std::cout << "left child of " << CurrentNode.Id << ": ";
			main_engine(CurrentNode.LeftOffset);
		}
	};
...
template <typename id , typename data>
long long hardMap<id , data>::put(const long long& CurrentPos, const hardNode& New){
